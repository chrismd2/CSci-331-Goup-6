...
void Block::getRecords()
{
  ...
  recordIndex.open("RecordOffsets.txt");
  postal.open("us_postal_codes.txt");
  offset = /*stoi(records[0]) * RECORDLENGTH + */POSTALHEADER;
  postal.seekg(offset);
  getline(postal, str);
  cout << "records[0]: " << records[0] << endl;
  cout << "str: " << str << endl;

  if(DEBUG) {cout << "Finished getRecords\n";}
}
